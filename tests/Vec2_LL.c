...
START_TEST(t_ll_InsertHead) {
    ...
    ck_assert(EqualV2(&LL_Get(&list, 0)->data, &(Vector2){2, 3}));
    ck_assert_int_eq(LL_GetCount(&list), 1);

    LL_InsertAtHead(&list, (Vector2){10, 16});
    ck_assert(EqualV2(&LL_Get(&list, 0)->data, &(Vector2){10, 16}));
    ck_assert(EqualV2(&LL_Get(&list, 1)->data, &(Vector2){2, 3}));
    ck_assert_int_eq(LL_GetCount(&list), 2);

    LL_InsertAtHead(&list, (Vector2){14, 14});
    ck_assert(EqualV2(&LL_Get(&list, 0)->data, &(Vector2){14, 14}));
    ck_assert(EqualV2(&LL_Get(&list, 1)->data, &(Vector2){10, 16}));
    ck_assert(EqualV2(&LL_Get(&list, 2)->data, &(Vector2){2, 3}));
...
START_TEST(t_ll_InsertTail) {
    ...
    ck_assert(EqualV2(&LL_Get(&list, 0)->data, &(Vector2){43, 13}));
    ck_assert_int_eq(LL_GetCount(&list), 1);

    LL_InsertAtTail(&list, (Vector2){12, 12});
    ck_assert(EqualV2(&LL_Get(&list, 0)->data, &(Vector2){43, 13}));
    ck_assert(EqualV2(&LL_Get(&list, 1)->data, &(Vector2){12, 12}));
    ck_assert_int_eq(LL_GetCount(&list), 2);

    LL_InsertAtTail(&list, (Vector2){14, 14});
    ck_assert(EqualV2(&LL_Get(&list, 0)->data, &(Vector2){43, 13}));
    ck_assert(EqualV2(&LL_Get(&list, 1)->data, &(Vector2){12, 12}));
    ck_assert(EqualV2(&LL_Get(&list, 2)->data, &(Vector2){14, 14}));
    ck_assert_int_eq(LL_GetCount(&list), 3);
}
END_TEST
